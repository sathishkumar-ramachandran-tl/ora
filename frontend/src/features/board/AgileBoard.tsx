...
import { Project, Task, Status, Priority, Note, TaskResource, Milestone, Sprint, BlockedTaskEntry, TaskDependencies } from '../../types';
import { Plus, Loader2, Sparkles, Pin, Play, Bot, X, Check, Book, Trash2, Save, FileText, ChevronRight, Paperclip, Link, MessageSquare, CalendarClock, Layout, Kanban, Tag, Bug, Star, BookOpen, Calendar, Lock, GitBranch, Milestone as MilestoneIcon, Wand2 } from 'lucide-react';
import { DndContext, DragEndEvent, DragOverlay, DragStartEvent, PointerSensor, useSensor, useSensors, useDroppable } from '@dnd-kit/core';
import { useDraggable } from '@dnd-kit/core';
import { generateProjectTasks } from '../../services/geminiService';
import { addTasks, updateTaskStatus, toggleTaskDailyFocus, updateTaskResources } from '../../api/tasks';
import { createNote, getNotes, deleteNote } from '../../api/workspace';
import {
  saveWhiteboard, getProjectMembers, assignUserToProject,
  getBlockedTasks, getTaskDependencies, addTaskDependency, removeTaskDependency,
  replanProject,
} from '../../api/projects';
import { createEvent } from '../../api/calendar';
import { CreateTaskModal } from '../../components/shared/CreationModals';
import { trackEvent } from '../../services/analytics';
import { SpatialCanvas } from '../canvas/SpatialCanvas';
import { PlanningChatDialog } from '../chat/PlanningChatDialog';
...
export const AgileBoard: React.FC<AgileBoardProps> = ({ project, companyMission, onUpdateProject, onStartFocus, onRequestRefresh }) => {
  const [viewMode, setViewMode] = useState<'kanban' | 'canvas' | 'milestones'>('kanban');
  ...
  // Project Management (Phase 3): sprint filter + dependency-blocked badges
  const [activeSprintId, setActiveSprintId] = useState<string | 'all'>('all');
  const [sprintsRefreshKey, setSprintsRefreshKey] = useState(0);
  const [blockedTaskMap, setBlockedTaskMap] = useState<Record<string, BlockedTaskEntry['blockedByTasks']>>({});
  ...
  const loadBlockedTasks = async () => {
    try {
      const blocked = await getBlockedTasks(project.id);
      const map: Record<string, BlockedTaskEntry['blockedByTasks']> = {};
      blocked.forEach(b => { map[b.taskId] = b.blockedByTasks; });
      setBlockedTaskMap(map);
    } catch (e) {
      console.error("Failed to load blocked tasks", e);
    }
  };
  ...
  const runReplan = async () => {
      if (!replanGoal.trim()) return;
      setIsReplanning(true);
      try {
          trackEvent('AI_REPLAN_START', { projectId: project.id });
          const result = await replanProject(project.id, replanGoal);
          setReplanSummary(result.summary);
          setReplanGoal("");
          if (onRequestRefresh) await onRequestRefresh();
          trackEvent('AI_REPLAN_COMPLETE', { projectId: project.id });
      } catch (e) {
          console.error("Replan failed", e);
          setReplanSummary("Replan failed. Please try again.");
      } finally {
    ...
    <div className="flex h-full gap-6 relative flex-col">
            ...
            <div className="flex items-center gap-3 flex-wrap">
                 <div className="flex bg-slate-100 p-1 rounded-lg">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${viewMode === 'milestones' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                ...
                {viewMode === 'kanban' && (
                    <SprintSelector
                        ...
                        onChange={setActiveSprintId}
                        refreshKey={sprintsRefreshKey}
                        onSprintsChanged={() => setSprintsRefreshKey(k => k + 1)}
                    />
                )}
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={() => { setReplanModalOpen(true); setReplanSummary(null); }}
                        className="flex items-center gap-2 bg-violet-50 text-violet-700 hover:bg-violet-100 px-3 py-2 rounded-lg text-sm font-medium transition-colors border border-violet-100 whitespace-nowrap"
                    >
                        <Bot size={16} /> AI Replan
                    </button>
                    <button
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {viewMode === 'canvas' ? (
            ...
            <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
                    ...
                    <div className="flex gap-4 h-full px-4 md:px-0 pb-4 md:min-w-[1000px]">
                        {COLUMNS.map(col => {
                            const tasks = project.tasks.filter(t =>
                                t.status === col.id && (activeSprintId === 'all' || t.sprintId === activeSprintId)
                            );
                            return (
                                <KanbanColumn key={col.id} id={col.id} label={col.label} count={tasks.length}>
                                    {tasks.map(task => (
                                        <TaskCard
                                            key={task.id}
                                            task={task}
                                            blockedBy={blockedTaskMap[task.id]}
                ...
                <DragOverlay>
                    {activeDragTask ? (
                        <div className="bg-white p-3 rounded-lg border border-indigo-300 shadow-lg w-72 rotate-2">
                            <h4 className="text-sm font-medium text-slate-800">{activeDragTask.title}</h4>
                        </div>
                    ) : null}
                </DragOverlay>
            </DndContext>
      ...
      {selectedTask && (
          <TaskDetailModal
            ...
            onDependencyChange={loadBlockedTasks}
          />
      )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 animate-in fade-in zoom-in-95">
            {isReplanning ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <h3 className="text-lg font-bold text-slate-800">Replanning project…</h3>
                    <p className="text-slate-500 text-sm mt-2 max-w-xs mx-auto">
                        The agent is updating milestones, sprints, and tasks for this project.
                    </p>
                </div>
            ) : replanSummary ? (
                <>
                    <h3 className="text-lg font-bold text-slate-800 mb-2 flex items-center gap-2"><Check className="text-emerald-500"/> Replan Complete</h3>
                    <p className="text-slate-600 text-sm whitespace-pre-wrap bg-slate-50 border border-slate-200 rounded-lg p-4">{replanSummary}</p>
                    <div className="flex justify-end mt-4">
                        <button onClick={() => setReplanModalOpen(false)} className="bg-slate-900 text-white px-4 py-2 rounded-lg font-medium hover:bg-slate-800">
                            Done
                        </button>
                    </div>
                </>
                ...
                <>
                    <h3 className="text-lg font-bold text-slate-800 mb-2 flex items-center gap-2"><Bot className="text-violet-500"/> AI Replan</h3>
                    <p className="text-slate-500 text-sm mb-4">
                        Describe the change and the agent will directly update this project's milestones, sprints, and tasks — e.g. "add a testing milestone before launch".
                    </p>
                    <textarea
                        className="w-full p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none min-h-[120px]"
                        placeholder="e.g. Add a testing milestone and push the launch date by a week..."
                        value={replanGoal}
                        onChange={(e) => setReplanGoal(e.target.value)}
                        autoFocus
                    />
                    <div className="flex justify-end gap-3 mt-4">
                        <button onClick={() => setReplanModalOpen(false)} className="text-slate-500 hover:text-slate-800 font-medium text-sm">Cancel</button>
                        <button onClick={runReplan} disabled={!replanGoal.trim()} className="bg-violet-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-violet-700 flex items-center gap-2 disabled:opacity-50">
                            Replan <Wand2 size={16} />
                        </button>
                    </div>
                </>
            )}
...
const KanbanColumn = ({ id, label, count, children }: { id: Status; label: string; count: number; children: React.ReactNode }) => {
    const { setNodeRef, isOver } = useDroppable({ id });
    return (
        <div
            ref={setNodeRef}
            className={`snap-center min-w-[85vw] sm:min-w-[320px] md:flex-1 flex flex-col rounded-xl border shadow-inner md:shadow-none transition-colors ${
                isOver ? 'bg-indigo-50 border-indigo-300' : 'bg-slate-100/50 border-slate-200/60'
            }`}
        >
            <div className="p-3 border-b border-slate-200/60 flex justify-between items-center bg-slate-50/50 rounded-t-xl sticky top-0 z-10 backdrop-blur-sm">
                ...
                <span className="text-xs text-slate-400 font-mono bg-slate-200/50 px-1.5 py-0.5 rounded">{count}</span>
            </div>
...
const TaskCard = ({ task, blockedBy, onOpen, onTogglePin, onStartFocus }: {
    task: Task;
    blockedBy?: { id: string; title: string; status: Status }[];
    onOpen: () => void;
    onTogglePin: (e: React.MouseEvent, taskId: string, currentStatus: boolean) => void;
    onStartFocus: (task: Task) => void;
}) => {
    ...
    return (
        <div
            ...
            {...attributes}
            onClick={onOpen}
            className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-all group cursor-grab active:cursor-grabbing active:scale-[0.98]"
        >
            <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-1 flex-wrap">
                    ...
                    {isBlocked && (
                        <span title={`Blocked by: ${blockedBy!.map(b => b.title).join(', ')}`} className="inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded bg-red-50 text-red-600 border border-red-100">
                            <Lock size={9} /> Blocked
                        </span>
                    )}
                </div>
                <div className="flex gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={(e) => onTogglePin(e, task.id, !!task.isDailyFocus)} className={`p-1 rounded hover:bg-slate-100 ${task.isDailyFocus ? 'text-amber-500 opacity-100' : 'text-slate-400'}`}>
...
const TaskDetailModal = ({ task, project, onClose, onUpdate, onDependencyChange }: {
    task: Task, project: Project, onClose: () => void, onUpdate: (t: Task) => void, onDependencyChange?: () => void
}) => {
    const [activeTab, setActiveTab] = useState<'overview' | 'notes' | 'resources' | 'dependencies'>('overview');
    ...
    const handleAddDependency = async () => {
        ...
        try {
            await addTaskDependency(task.id, dependencyTargetId);
            setDependencyTargetId('');
            await loadDependencies();
            onDependencyChange?.();
        } catch (e: any) {
            alert(e?.response?.data?.error || "Failed to add dependency.");
        }
    };
    ...
    const isBlocked = !!dependencies?.dependsOn.some(d => d.task && d.task.status !== 'done');
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex justify-end">
            <div className="w-full md:w-[600px] bg-white h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
                ...
                <div className="flex border-b border-slate-200 px-6 gap-6">
                    {['overview', 'notes', 'resources', 'dependencies'].map((tab) => (
                        <button
                            key={tab}
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
                    ...
                    {activeTab === 'dependencies' && (
                        <div className="space-y-6">
                            {isBlocked && (
                                <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg p-3 text-sm flex items-center gap-2 font-medium">
                                    <Lock size={16} /> This task is blocked until its dependencies are done.
                                </div>
                            )}
                            {loadingDeps ? (
                                ...
                                <>
                                    <div className="space-y-3">
                                        <h3 className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1"><GitBranch size={12} /> Blocks (this task depends on)</h3>
                                        {(!dependencies || dependencies.dependsOn.length === 0) && (
                                            <div className="text-sm text-slate-400 italic bg-white p-4 rounded-lg border border-slate-200 border-dashed text-center">
                                                No dependencies.
                                            </div>
                                        )}
                                        {dependencies?.dependsOn.map(dep => (
                                            <div key={dep.dependencyId} className="flex items-center justify-between bg-white p-3 rounded-lg border border-slate-200">
                                                <div>
                                                    <p className="text-sm font-medium text-slate-800">{dep.task?.title || 'Unknown task'}</p>
                                                    <p className="text-xs text-slate-400 capitalize">{dep.task?.status.replace('-', ' ')}</p>
                                                </div>
                                                <button onClick={() => handleRemoveDependency(dep.dependencyId)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded">
                                                    <Trash2 size={14} />
                                                </button>
                                            </div>
                                        ))}
                                    ...
                                    <div className="bg-white p-4 rounded-xl border border-slate-200">
                                        <h3 className="text-sm font-bold text-slate-800 mb-3">Add Dependency</h3>
                                        <div className="flex gap-2">
                                            <select
                                            ...
                                            <button onClick={handleAddDependency} disabled={!dependencyTargetId} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50">
                                                Add
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        <h3 className="text-xs font-bold text-slate-500 uppercase">Blocked By This Task</h3>
                                        {(!dependencies || dependencies.blockedBy.length === 0) && (
                                            <div className="text-sm text-slate-400 italic bg-white p-4 rounded-lg border border-slate-200 border-dashed text-center">
