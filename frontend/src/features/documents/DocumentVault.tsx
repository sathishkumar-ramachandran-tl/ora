...
export const DocumentVault: React.FC<DocumentVaultProps> = ({ workspaceId }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newDoc: Document = { ...response.data, uploadedAt: new Date(response.data.uploadedAt) };
  ...
  const handleDownloadDocument = async (docId: string) => {
    try {
      ...
      setError(e.response?.data?.error || 'Failed to download document');
    }
  };
                    ...
                    <div key={doc.id} className="group border border-slate-200 rounded-xl p-4 hover:shadow-md transition-shadow bg-white relative">
                        ...
                        <div className="absolute inset-0 bg-white/50 opacity-0 group-hover:opacity-100 flex items-center justify-center backdrop-blur-[1px] transition-opacity rounded-xl">
                            <button
                              ...
                              className="bg-white text-indigo-600 shadow-lg border border-slate-100 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transform translate-y-2 group-hover:translate-y-0 transition-transform">
